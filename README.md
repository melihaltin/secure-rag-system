...
### 👉 [Click Here to Chat with Secure HR Guard](https://secure-rag.melih.app)

_(Try asking for the CEO's salary to see the security rails in action!)_
...
_Figure 1: Automated Security Audit showing successful blocking of sensitive data requests._
...
_Figure 2: Ragas evaluation results showing high context recall and precision._

> **🔍 Critical Insight on "Failures":**
> You may notice a lower score/fail status on the _Alexander Kensington_ query in Figure 2.
>
...
- The system uses **Context-Aware Filtering**. Even if the `Retriever` fetches a document containing salary info, the Guardrails layer validates if the user has the _intent_ authorization to see it.
...
> _Note: In a production environment, these logs would be piped to a SIEM tool (e.g., Datadog, Splunk) instead of a CSV file._
